...
:: @about Скрипт для оптимизации видео в формате webm сайта Лаборатории.
:: @purpose Выполняет пакетный поиск неоптимизированных видеороликов .webm, выявляет файлы с поврежденным или отсутствующим индексом длительности (duration=N/A) с помощью ffprobe, резервирует место под метаданные (добавляет 1024K индексного пространства через ffmpeg) для быстрого старта видео при загрузке сайта и автоматически сохраняет оригиналы в безопасную папку webm-sources.
:: @author TechLab
