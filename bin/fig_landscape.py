...
@about Полный независимый модуль для обработки одиночных журнальных блоков.
...
def process_single_figure_landscape(markdown_content):
    ...
    # 🎯 ЭТАП Б: ОБРАБОТКА ЦЕЛЕВЫХ ССЫЛОК (ИСПРАВЛЕННЫЙ ТОЧНЫЙ ПАТТЕРН)
    # Находит ![, затем собирает всё до скобки ], если внутри есть {fig, и забирает URL из ()
    pattern = r'!\[([^\]]*?\{fig[^\]]*?\)\]\(([^)]*?)\)'
