...
def build_sitemap_tree():
        ...
        if os.path.isdir(full_path) and name not in EXCLUDED_FOLDERS and not name.startswith('.') and name != '_posts':
            ...
            for root_walk, _, files in os.walk(full_path):
                for file in files:
                    ...
                    # УДАЛЕН ВЫЗОВ НА ЭТАЖ 2. Очищенная нода сразу идет в flat_map
    ...
    try:
        ...
        print("[SITEMAP-SUCCESS] Карта sitemap.yml успешно сохранена на диск.")
    except Exception as e:
        print(f"[NAV-ERROR] Ошибка записи карты навигации sitemap.yml: {e}")
