...
"""
@module vault
@about Автономный сквозной точечный сейф исключений с отладочным логом сырого контента.
@purpose Сканирует текст, изолирует опасные маркеры внутри кодовых блоков/комментариев,
         выводит сырое состояние строк на входе и возвращает их обратно.
@author TechLab
@version 1.1-raw-debug
"""
...
def global_freeze_content(markdown_content, file_rel_path):
    ...
    Выводит сырой текст в консоль для анализа автозамен.
    """
    global_vault = []
    ...
    def freezer(match, block_type):
        ...
        # 🛠️ [ОТЛАДКА] ВЫВОД ТОЧНОГО СЫРОГО ТЕКСТА ИЗ ФАЙЛА ДО ВСЕХ ОБРАБОТОК
        print(f"\n[VAULT-RAW-DBG] ИСХОДНЫЙ ТЕКСТ ИЗ ФАЙЛА: {raw_text}")
