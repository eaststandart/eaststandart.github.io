---
layout: page
description: Развитие навыков конструирования комбинированных корпусов из дерева и пластика, создание встроенных электрических схем. Изучение принципов работы несимметричного мультивибратора и подбора частотозадающих компонентов для управления звуком.
title: Бластер электроакустический
tags: [электроникаисхемотехника, проектучебный, 4класс, проект, бластерэлектроакустический, electroacousticblaster]
sources: "[Walking Robot](https://blackfishspace.com/product/walking-robot-templates-pdf/)"
date: 2026-01-24
permalink: /faire/blaster-elektroakusticheskij/
---

### Описание проекта
Создание конструкции электроакустического бластера на основе схемы несимметричного мультивибратора с использованием германиевых транзисторов. Устройство собрано на эргономичной деревянной рукоятке с ручным регулятором частоты. Для усиления звука ствол выполнен из пластиковой трубы, в которую интегрированы динамик  и блок из четырех светодиодов, мигающих в такт акустическим импульсам. Использование советских ретро-деталей в открытом монтаже придает бластеру уникальный футуристический образ.

![](blaster-elektroakusticheskij-1.webp)
![](blaster-elektroakusticheskij-2.webp)
![](blaster-elektroakusticheskij-3.webp)

> **Смотри также:**
> 1\. [Несимметричный мультивибратор на транзисторах](https://www.chipdip.ru/video/id000553663).\
> 2\. [Несимметричный генератор. Как это работает и несколько самоделок из него](https://yandex.ru/video/preview/2135205737810259244).

### Область применения
Использование в качестве уникального игрового реквизита и наглядного пособия по изучению аналоговой схемотехники. Для юного космонавта это индивидуальное защитное устройство, генерирующее направленные звуковые и световые импульсы для ориентации в темных пещерах и подачи сигналов на неизведанных планетах.

![video](blaster-elektroakusticheskij-1.webm)

### Развитие проекта
Добавление в схему лазерного модуля для подсветки цели или генерации ярких вспышек в такт частоте выстрелов бластера.

### Файлы проекта
1\. 📄[Сборочный чертеж, PDF](blaster-elektroakusticheskij.pdf)\
2\. 📐[Сборочный чертеж, LibreCAD](blaster-elektroakusticheskij.dxf)

### Журнал проекта
<!-- *Назначение: этапы создания, отладка и усовершенствование.* -->
{% include posts-page.liquid type="journal" per_page="" %}

### Галерея работ
<!-- *Назначение: демонстрация (фото, видео) выполненного проекта от участников.* -->
{% include posts-page.liquid type="media" %}
