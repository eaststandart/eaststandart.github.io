---
layout: page
title: Музыкальный карандаш
description: "Развитие навыков чтения электронных схем и понимание принципов их работы. Изучение работы биполярных транзисторов в стандартном и инверсном режимах, процессов заряда и разряда конденсаторов, Развитие базовых навыков конструирования, монтажа и настройки электронных устройств. Изучение основ схемотехники и чтение принципиальных электрических схем для понимания функционирования устройств. Принципы построения аналоговых звуковых синтезаторов на базе интегральной микросхемы-таймера 555. Освоение практической работы с транзисторами из кремния и германия для глубокого изучения физики процессов."
author: ["RadiotekhnikaClub"]
tags: [электроникаисхемотехника, проектучебный, 4класс, проект, карандашмузыкальный, схемаэлектрическаяпринципиальная, электрическоеискусство, electronicsart]
sources: "[“Drawdio” Musical Pencil](https://makezine.com/projects/drawdio-musical-pencil/)"
date: 2026-04-13
permalink: /faire/muzykalnyj-karandash/
---
  
### Описание проекта
Создание простого электронного звукового синтезатора, который использует проводящие свойства графита и человеческого тела для воспроизведения музыкального тона с изменяющейся частотой. Благодаря этому устройству можно создавать различные звуки, рисовать музыкальные инструменты на обычном листе бумаги и играть на них небольшие мелодии. Также прибор позволяет превратить в музыкальные инструменты любые окружающие предметы, например кисти для рисования, стол, деревья, фрукты или кухонную раковину.


![](github/eaststandart.github.io/faire/muzykalnyj-karandash/muzykalnyj-karandash-01.webp)
![v](github/eaststandart.github.io/faire/muzykalnyj-karandash/muzykalnyj-karandash-02.webp)

> **Примечание.** 

### Область применения
В бытовых условиях устройство может использоваться как необычный музыкальный инструмент для творчества и проведения весёлых научных опытов с друзьями или служить простейшим прибором для исследования электрической проводимости окружающих предметов и материалов путем преобразования их сопротивления в слышимый человеком звук. В будущем этот принцип может применяться в исследовательских приборах для экспресс-анализа почвы и других материалов на поверхности планет. Подобная звуковая индикация поможет космонавтам-исследователям мгновенно определять состав и влажность грунта при поиске полезных ископаемых или подземного льда в космических экспедициях.

![video](github/eaststandart.github.io/faire/muzykalnyj-karandash/muzykalnyj-karandash-01.webm)

> **Примечание.**.

### Развитие проекта
1\. Установка усилителя звука на двух транзисторах для увеличения громкости и чистоты звучания прибора.\
2\. Создание таблицы соответствия уровня проводимости различным материалам для точного определения типа исследуемого вещества.\
3\. Проведение экспериментов по проверке качества строительных материалов и определению марок бетона в зависимости от их электрического сопротивления.\
4\. Поиск скрытых трещин и дефектов в металлических конструкциях по резкому изменению тональности звука при прохождении датчика над повреждением.\
5\. Использование прибора в качестве бесконтактного детектора влажности почвы комнатных растений для определения точного времени их полива по высоте звукового тона.\
6\. Применение устройства на уроках химии для демонстрации разницы электропроводности между чистой дистиллированной водой и различными растворами солей или кислот.

### Файлы проекта
1\. 📄[Схема электрическая принципиальная, PDF](muzykalnyj-karandash-esd.pdf)\
2\. 🖼️[Схема электрическая принципиальная, JPG](muzykalnyj-karandash-esd.jpg)

![v](github/eaststandart.github.io/faire/muzykalnyj-karandash/muzykalnyj-karandash-03.webp)

### Журнал проекта
<!-- *Назначение: этапы создания, отладка и усовершенствование.* -->
{% include posts-page.liquid type="journal" per_page="" %}

### Галерея работ
<!-- *Назначение: демонстрация (фото, видео) выполненного проекта от участников.* -->
{% include posts-page.liquid type="media" %}

{% comment %}  Ссылки на статьи в /\_posts по этому проекту
[[../../_posts/2026-04-14-muzykalnyj-karandash|2026-04-14-muzykalnyj-karandash]]
[[../../_posts/2026-04-15-muzykalnyj-karandash|2026-04-15-muzykalnyj-karandash]]
[[../../_posts/2026-04-16-muzykalnyj-karandash|2026-04-16-muzykalnyj-karandash]]
[[../../_posts/2026-04-17-muzykalnyj-karandash|2026-04-17-muzykalnyj-karandash]]
[[../../_posts/2026-04-18-muzykalnyj-karandash|2026-04-18-muzykalnyj-karandash]]
[[../../_posts/2026-04-19-muzykalnyj-karandash|2026-04-19-muzykalnyj-karandash]]
[[../../_posts/2026-04-20-muzykalnyj-karandash|2026-04-19-muzykalnyj-karandash]]
{% endcomment %}