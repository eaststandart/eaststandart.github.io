...
<div class="grid-container">
    ...
    <section class="category-card site-news">
        ...
        <ul id="live-updates-list" style="list-style: none; padding: 0; margin: 0;">
            {% comment %} Объединяем ВСЕГДА посты, страницы и людей в один массив {% endcomment %}
            ...
            {% for item in all_content %}
              {% if item.date and item.url != "/" and item.url != "/tags.html" and item.url != "/news/" %}
                ...
                {% comment %} Автоматически вырезаем первую папку (для роботов это journal/media, для людей — people) {% endcomment %}
                ...
                {% comment %} Ищем родительскую страницу раздела (включая _pages/people) {% endcomment %}
...
{% include pagination.liquid list_id="live-updates-list" controls_id="home-news-pagination" per_page=10 pinned_url="/people/fran-blanche/" %}
