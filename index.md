        ...
        <ul id="live-updates-list" style="list-style: none; padding: 0; margin: 0;">
            ...
            {% for item in all_content %}
              {% if item.date and item.url != "/" and item.url != "/tags.html" and item.url != "/news/" %}
                ...
                <li class="update-item news-item-compact" data-date="{{ item.date | date: '%Y-%m-%d' }}" data-is-post="{{ is_post }}" data-is-person="false" style="display: none;">      
            ...
            {% comment %} ДОБАВЛЕНО: Безопасный вывод людей с сохранением твоих оригинальных классов {% endcomment %}
            {% for person in site.people %}
              {% if person.date %}
                <li class="update-item news-item-compact" data-date="{{ person.date | date: '%Y-%m-%d' }}" data-is-post="false" data-is-person="true" style="display: none;">      
