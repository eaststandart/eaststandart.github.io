        ...
        <ul id="live-updates-list" style="list-style: none; padding: 0; margin: 0;">
            ...
            {% for item in all_content %}
              {% if item.date and item.url != "/" and item.url != "/tags.html" and item.url != "/news/" %}
                ...
                {% comment %} 
                  МАГИЯ LIQUID: Вычисляем родительский раздел по URL новости 
                  и вытаскиваем из его Front Matter параметр emoji
                {% endcomment %}
                {% assign item_section = item.url | split: "/" | first | append: "/" | prepend: "/" %}
                {% if item.url contains "/people/" %}
                  {% assign item_section = "/people/" %}
                {% elsif item.path contains '_posts' %}
                  {% comment %} Для обычных постов журналов берем их категорию как раздел {% endcomment %}
            ...
            {% comment %} 
              2. Цикл для коллекции людей (так как они лежат отдельно, 
              мы сразу ищем страницу /people/ и забираем её эмодзи)
            {% endcomment %}
